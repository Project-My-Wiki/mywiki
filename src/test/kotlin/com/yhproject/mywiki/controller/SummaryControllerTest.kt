...
class SummaryControllerTest {
        ...
        fun `createSummary returns created summary`() {
                ...
                val testContentItems =
                        listOf(SummaryContentItem(id = 1, content = "Test summary content"))
                val request =
                        SummaryCreateRequest(bookmarkId = bookmarkId, contents = testContentItems)
        ...
        fun `updateSummary returns updated summary`() {
                ...
                val bookmark = createTestBookmark(bookmarkId, userId)
                val summary =
                        createTestSummary(summaryId, bookmark, SummaryContents(testContentItems))
                val templates =
                        listOf(
                                createTestTemplate(
                                        1,
        ...
        fun `getSummaryByBookmark returns a summary`() {
                ...
                val templates =
                        listOf(
                                createTestTemplate(
                                        3,
                                        SummaryTemplateSection.DETAILS,
                                        "왜-무엇을-어떻게형",
                                        ""
                                )
                        )
                ...
                whenever(summaryService.getSummaryByBookmarkId(bookmarkId, userId))
                        .thenReturn(summary)
                whenever(summaryService.getSummaryTemplates()).thenReturn(templates)
